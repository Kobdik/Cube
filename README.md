...
# Делай аналитику в браузере!

Kobdik - инструмент бизнес-аналитики (BI), написанный заново с нуля, с целью реализовать новую парадигму по формированию сводных отчётов в разрезе иерархических измерений (деревьев) прямо в браузере на стороне клиента. 
...
Попробуйте интерактивную демонстрацию бюджета затрат. Регистрация не нужна, данные в количестве 29 160 записей сгенерированы по 4-м измерениям. Выберите в каждом измерении некоторые элементы, например, кликните по полям (рядом с текстом): 1-й квартал, План 2025, Материальные затраты, Подразделение 1 и в результате получите четыре связанные сводные таблицы заполненные данными.

Используте колесико мыши для перемотки окна по вертикали и по горизонтали (при зажатой клавише Shift).

Переходите к интерактивному демо: [Бюджет затрат](./pfa.html)

Пример использования инструмента глазами аналитика с подробным описанием сценарного анализа в случае возможного срыва сроков поставки материалов: [Анализ движения материалов](./flow.md)
...
5. гибкий факт-план анализ (actual vs. forecast analysis)
...
Далее перечислены перспективы развития библиотеки и её применений.
...
## Разработка редактора обобщенных измерений

Пример JSON-описания обобщенных измерений: [ords.json](./cashed/ords.json)
Подробное описание структуры обобщенных измерений напишу позднее.
...
## Разработка расчётного движка на Golang

Алгоритмы расчёта сводных отчётов могут быть переписаны на Golang и размещены на бэкенде. Возможности системы кратно возрастут, как по скорости расчётов, так и по свободе и гибкости модели, интеграциям с другими системами, это не считая выразительности самого языка программирования. Kobdik на бэке позволит организовать совместную работу тысяч клиентов через Интернет.
