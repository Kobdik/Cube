...
Количество аналитических данных на вход можно существенно ограничить с помощью Materialized View, предварительно расчитав агрегаты по всем вариациям ключей с помощью GROUP BY (Col1, Col2, Col3, ..., ColN). Количество таких агрегатов будет зависеть только от количества и глубины измерений, но, в подавляющем большинстве ситуаций, легко разместится в оперативной памяти. Поэтому, даже в случае миллиардов анализируемых данных, предварительное кэширование агрегатов по ключам всех измерений позволит применять In-Memory OLAP обработку данных.

Конечно, размер матрицы сводного отчёта может оказаться не в пределах 70 X 20 = 1 400, а в пределах 70 X 800 = 56 000 ячеек, если в календарные периоды добавить пару лет с детализацией до дней. Однако, что находится в пределах 100 тысяч ячеек находится в рабочем диапазоне для отрисовки в SVG.
