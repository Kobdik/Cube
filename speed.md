...
Количество аналитических данных на вход можно существенно ограничить с помощью Materialized View, предварительно расчитав агрегаты по вариациям ключей всех измерений с помощью GROUP BY (Col1, Col2, Col3, ..., ColN). Количество таких агрегатов будет зависеть только от количества, кардинальности (количества элементов) и глубины измерений, и в подавляющем большинстве ситуаций, легко разместятся в оперативной памяти. Поэтому, даже в случае миллиардов анализируемых данных, предварительное кэширование агрегатов по ключам всех измерений позволит применять In-Memory OLAP обработку данных.
